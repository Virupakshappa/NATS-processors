...
public class Connection {
	...
	protected static void init(Properties popts) {
		...
		if (!popts.containsKey("ssl")) popts.put("ssl", Boolean.FALSE);
		if (!popts.containsKey("max_reconnect_attempts")) popts.put("max_reconnect_attempts", Integer.valueOf(DEFAULT_MAX_RECONNECT_ATTEMPTS));
		if (!popts.containsKey("reconnect_time_wait")) popts.put("reconnect_time_wait", Integer.valueOf(DEFAULT_RECONNECT_TIME_WAIT));
		...
		if (System.getenv("NATS_VERBOSE") != null) popts.put("verbose", Boolean.valueOf(System.getenv("NATS_VERBOSE")));
		if (System.getenv("NATS_PEDANTIC") != null) popts.put("pedantic", Boolean.valueOf(System.getenv("NATS_PEDANTIC")));
		if (System.getenv("NATS_DEBUG") != null) popts.put("debug", Boolean.valueOf(System.getenv("NATS_DEBUG")));
		if (System.getenv("NATS_RECONNECT") != null) popts.put("reconnect", Boolean.valueOf(System.getenv("NATS_RECONNECT")));
		if (System.getenv("NATS_FAST_PRODUCER") != null) popts.put("fast_producer", Boolean.valueOf(System.getenv("NATS_FAST_PRODUCER")));
		if (System.getenv("NATS_SSL") != null) popts.put("ssl", Boolean.valueOf(System.getenv("NATS_SSL")));
	...
	private void sendSubscriptions() throws IOException {
		...
		for(Iterator<Entry<Integer, Subscription>> iter = subs.entrySet().iterator(); iter.hasNext();) {
			entry = iter.next();
